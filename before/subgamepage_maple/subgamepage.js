...
popUpDone.onclick = function () {
  ...
  document
    ...
    .appendChild(mainWriteContentsImgWrapper);
  ...
  mainWriteContentsImgWrappersArr = document.querySelectorAll(
    ".mainWriteContentsImgWrapper"
  );
  mainWriteContentsImgBtnsArr = document.querySelectorAll(
    ".mainWriteContentsImgBtn"
  );
  ...
  for (let i = 0; i < mainWriteContentsImgWrappersArr.length; i++) {
    mainWriteContentsImgBtnsArr[i].onclick = function () {
      document
        .querySelector(".mainWriteContents")
        .removeChild(mainWriteContentsImgWrappersArr[i]);
    };
  }
...
mainWriteText.onkeydown = function () {
...
forlogininput[0].onkeydown = function (e) {
  ...
  if (e.keyCode == 13) {
    if (
      forlogininput[0].value == "admin" &&
      forlogininput[1].value == "admin"
    ) {
      alert("테스트 모드입니다.");
