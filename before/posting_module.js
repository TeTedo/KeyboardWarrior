...
let wholeGameSlideNextButton = document.querySelectorAll(
    ".wholeGameSlideNextButton"
);
let wholeGameSlidePrevButton = document.querySelectorAll(
...
let PostGnb = (function () {
    ...
    PostGnb.prototype.init = function (super_id, text, imgUrlsArr) {
        ...
        //현재시간 넣어주기
        tempprofileTime.innerHTML =
            tempDate + " " + tempHours + ":" + tempMinutes;

        //댓글창
        temprecommentBoxinput.setAttribute("onkeydown", "resize2(this)");
        temprecommentBoxinput.setAttribute("onkeyup", "resize2(this)");
        ...
        if (!(imgUrlsArr.length === 0)) {
            for (let i = 0; i < imgUrlsArr.length; i++) {
                ...
                postWriteContentsImgWrapper.appendChild(
                    postWriteContentsImgBtn
                );
...
let renewalArr = function () {
    ...
    wholeGameSlideNextButton = document.querySelectorAll(
        ".wholeGameSlideNextButton"
    );
    wholeGameSlidePrevButton = document.querySelectorAll(
        ".wholeGameSlidePrevButton"
    );
    ...
    for (let i = 0; i < postWritesArr.length; i++) {
        ...
        modifyBtnsArr[i].onclick = function (e) {
            ...
            if (e.target.innerText === "수정") {
                ...
                //이미지 리스트형식으로 보이게하고
                let Imgtemp = postWriteContentsArr[i].querySelectorAll(
                    ".postWriteContentsImgWrapper"
                );
                ...
                //슬라이드안보이게하고
                postWriteContentsArr[i].querySelector(
                    ".wholeGameSlide"
                ).style.display = "none";
                //사진위에x버튼 보이게하고
                buttonDisplay(i);
            ...
            //완료버튼 눌렀을때
                ...
                for (
                    let k = 0;
                    k < postWriteContentsArr[i].children.length - 3;
                    k++
                ) {
                    postWriteContentsArr[i].children[
                        k + 3
                    ].children[0].style.display = "none";
                }
                //일반사진들 안보이게하고
                let imgCount = postWriteContentsArr[i].querySelectorAll(
                    ".postWriteContentsImgWrapper"
                );
                ...
                //먼저 원래있던 slidUl 날림
                tempUl =
                    postWriteContentsArr[i].querySelector(".wholeGameSlideul");
                postWriteContentsArr[i]
                    .querySelector(".wholeGameSlide")
                    .removeChild(tempUl);
                //slideUl 새로 만들어서 넣음
                ...
                for (let m = 0; m < imgCount.length; m++) {
                    ...
                    tempGameSlidelist.classList.add("wholeGameSlidelist");
                    let tempGameSlideImg = document.createElement("img");
                    postWriteContentsImg =
                        postWriteContentsArr[i].children[m + 3].children[1];
                    tempGameSlideImg.src = postWriteContentsImg.src;
        ...
        modifyAddImgBtnsArr[i].onclick = function () {
            ...
            ModifyPopUpDone.onclick = function () {
                ...
                let tempmodifyTextareaWrap = document.querySelectorAll(
                    ".modifyTextareaWrap"
                );
                ...
                postWriteContentsArr[i].appendChild(
                    postWriteContentsImgWrapper
                );
                postWriteContentsImgWrapper.appendChild(
                    postWriteContentsImgBtn
        ...
        function modifiyImgDelete() {
            for (
                let m = 0;
                m < postWriteContentsArr[i].children.length - 3;
                m++
            ) {
                postWriteContentsArr[i].children[m + 3].children[0].onclick =
                    function (e) {
                        if (
...
function buttonDisplay(i) {
    for (let k = 0; k < postWriteContentsArr[i].children.length - 3; k++) {
        postWriteContentsArr[i].children[k + 3].children[0].style.display =
            "block";
    }
}
        ...
        wholeGameSlide[i].onclick = function (e) {
            if (e.target.classList.contains("wholeGameSlideNextButton")) {
                ...
                if (index > wholeGameSlideul[i].childElementCount - 1) {
                    ...
                    wholeGameSlideul[i].style.left =
                        parseInt(wholeGameSlideul[i].style.left) - 350 + "px";
                }
            }
            if (e.target.classList.contains("wholeGameSlidePrevButton")) {
                ...
                if (index < 0) {
                    ...
                    wholeGameSlideul[i].style.left =
                        -350 * (wholeGameSlideul[i].childElementCount - 1) +
                        "px";
                } else {
                    wholeGameSlideul[i].style.left =
                        parseInt(wholeGameSlideul[i].style.left) + 350 + "px";
...
postRegisterBtn.onclick = function () {
    ...
    if (imgUrlsArr.length === 0) {
        ...
        for (let i = 0; i < removeImgs.length; i++) {
            document
                .querySelector(".mainWriteContents")
                .removeChild(removeImgs[i]);
        }
    }
    ...
    for (let i = 0; i < postWritesArr.length; i++) {
        ...
        likediv[i].onclick = function () {
            if (fullheartStatus == 0) {
                ...
                likedivcount[i].innerHTML =
                    Number(likedivcount[i].innerHTML) + 1;
                fullheartStatus += 1;
            } else {
                //좋아요 취소
        ...
        postHashTagtoClick[i].onclick = function () {
            if (!postHashTagtoClick[i].classList.contains("selectedCategory")) {
                ...
                for (let j = 0; j < postWritesArr.length; j++) {
                    let tofindHastag =
                        postWritesArr[j].querySelector(".postWriteHashTag");
                    if (
                        tofindHastag.innerHTML !=
                        postHashTagtoClick[i].innerText
                    ) {
...
let addRecommentFun = function () {
    for (let i = 0; i < postWritesArr.length; i++) {
        recommentPost[i].onclick = function () {
            ...
            temprecommentrepositoryWrap.classList.add(
                "recommentrepositoryWrap"
            );
            ...
            let temprecommentrepositoryTimeStamp =
                document.createElement("div");
            temprecommentrepositoryTimeStamp.classList.add(
                "recommentrepositoryTimeStamp"
            );
            let temprecommentrepositoryModify = document.createElement("div");
            temprecommentrepositoryModify.classList.add(
                "recommentrepositoryModify"
            );
            let temprecommentrepositoryDelete = document.createElement("div");
            temprecommentrepositoryDelete.classList.add(
                "recommentrepositoryDelete"
            );
            temprecommentComment.setAttribute("onkeydown", "resize2(this)");
            temprecommentComment.setAttribute("onkeyup", "resize2(this)");
            temprecommentComment.setAttribute("rows", "1");
            temprecommentComment.setAttribute("readonly", "");
            temprecommentComment.style.height = `${recommentBoxinput[i].scrollHeight}px`;
            ...
            temprecommentrepositoryWrap.appendChild(
                temprecommentrepositoryWrapimg
            );
            temprecommentrepositoryWrap.appendChild(temprecommentComment);
            temprecommentrepositoryWrap.appendChild(
                temprecommentrepositoryTimeStamp
            );
            temprecommentrepositoryWrap.appendChild(
                temprecommentrepositoryModify
            );
            temprecommentrepositoryWrap.appendChild(
                temprecommentrepositoryDelete
            );
...
//댓글 수정
let recommentrepositoryModify = document.querySelectorAll(
    ".recommentrepositoryModify"
);

let modifyDeleteRecomment = function () {
    for (let i = 0; i < postWritesArr.length; i++) {
        let temprecommentrepositoryModify = postWritesArr[i].querySelectorAll(
            ".recommentrepositoryModify"
        );
        let temprecommentComment =
            postWritesArr[i].querySelectorAll(".recommentComment");
        let temprecommentrepositoryDelete = postWritesArr[i].querySelectorAll(
            ".recommentrepositoryDelete"
        );
...
let mainSearchFun = function () {
    topSearchBtn.onclick = function () {
        ...
        for (let i = 0; i < postWritesArr.length; i++) {
            let tosearch =
                postWritesArr[i].querySelector(".modifyTextarea").value;
