...
pickbutton.onclick = function(){
    ...
    setTimeout(()=>{
        ...
        pickedElement.style.left = 550 - index_crane*50 + "px";
        pickedElement.style.bottom = "-100px";
    },1500)

    //인형이 바구니로 떨어짐
    setTimeout(()=>{
        pickedElement.style.bottom = 450 - bowlcolumn.childElementCount*50 + "px";
    },2000)
    //초기로 이동 + 바구니에 img 추가
    setTimeout(()=>{
        cranehand.style.left = "50px"
        bowlcolumn.appendChild(column[index_crane].lastChild);
        index_crane = -1;
    },2500)
    //같은개 2개면 제거 + 점수 오름
    setTimeout(()=>{
        let bowlcolumnobject = document.querySelectorAll(".picked");
