  ...
  for (let i = 0; i < postWritesArr.length; i++) {
    ...
    modifyBtnsArr[i].onclick = function (e) {
      ...
      if (e.target.innerText === "수정") {
        ...
        //슬라이드안보이게하고
        postWriteContentsArr[i].querySelector(".wholeGameSlide").style.display =
          "none";
        //사진위에x버튼 보이게하고
        buttonDisplay(i);
    ...
    function modifiyImgDelete() {
      for (let m = 0; m < postWriteContentsArr[i].children.length - 3; m++) {
        postWriteContentsArr[i].children[m + 3].children[0].onclick = function (
          ...
          if (
            e.target.classList.contains("wholeGameSlideNextButton") ||
            e.target.classList.contains("wholeGameSlidePrevButton")
          ) {
            return;
          }
    ...
    wholeGameSlide[i].onclick = function (e) {
      ...
      if (e.target.classList.contains("wholeGameSlidePrevButton")) {
        ...
        if (index < 0) {
          ...
          wholeGameSlideul[i].style.left =
            -350 * (wholeGameSlideul[i].childElementCount - 1) + "px";
        } else {
          wholeGameSlideul[i].style.left =
            parseInt(wholeGameSlideul[i].style.left) + 350 + "px";
        }
